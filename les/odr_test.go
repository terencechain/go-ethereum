	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				vmenv := vm.NewEVM(context, txContext, statedb, config, new(vm.Config))

				//vmenv := core.NewEnv(statedb, config, bc, msg, header, new(vm.Config))
			...
			txContext := core.NewEVMTxContext(msg)
			vmenv := vm.NewEVM(context, txContext, state, config, new(vm.Config))
			gp := new(core.GasPool).AddGas(math.MaxUint64)
			result, _ := core.ApplyMessage(vmenv, msg, gp)
			if state.Error() == nil {
				res = append(res, result.Return()...)
